...
contract Utils is OwnableUpgradeable, PausableUpgradeable {
    ...
    function random(uint256 from, uint256 to)
        public
        view
        whenNotPaused
        returns (uint256 number)
    {
        ...
        uint256 tmp3 = uint256(
            keccak256(
                abi.encodePacked(block.timestamp, randomNumber, msg.sender)
            )
        ) % 3333;
        uint256 tmp4 = uint256(
            keccak256(
    ...
    function getRandomNumber(uint256 _privateNumber)
        public
        view
        returns (uint256)
    {
